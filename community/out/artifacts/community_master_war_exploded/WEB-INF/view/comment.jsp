            ...
            <li class="row">
                <span>
                    <a class="text-primary">${comment.username}</a>&nbsp;
                        <c:if test="${comment.cforId!=null}">
                            回复&nbsp;<a class="text-primary">${comment.cforUsername}</a>
                        </c:if>
                        :&nbsp;${comment.content}
                </span>
                <div class="flex-grow-1 d-flex justify-content-end mr-3">
                    <!--fmt:formatDate 按照指定格式格式化时间标签-->
                    <span class="time"><fmt:formatDate value="${comment.time}" pattern="yyyy-MM-dd hh:mm"/></span>
                    <!--href="javascript:void(0)" 死链接，无反应-->
                    <a class="ml-1 time" onclick="toCommentFor(${cinId},${comment.userId},'${comment.username}')" href="javascript:void(0)">回复</a>
                </div>
            </li>
        </c:forEach>
    </ul>
